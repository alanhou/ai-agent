...
import (
	...
	"agents-go/go/internal/scenarios/it_help_desk"
...
func main() {
	...
	agent, err := it_help_desk.NewAgent(ctx)
	if err != nil {
		fmt.Fprintf(os.Stderr, "Failed to create agent: %v\n", err)
		os.Exit(1)
	}

	var initialState *it_help_desk.AgentState
	...
	if *evalMode {
		...
		initialState = &it_help_desk.AgentState{}
		// Note: The evaluation script sends {"ticket": ..., "messages": ...} which matches AgentState json tags.
		...
		initialState = &it_help_desk.AgentState{
			...
			Messages: []*schema.Message{
				schema.UserMessage("I cannot access the VPN from home."),
			},
		}
	}
