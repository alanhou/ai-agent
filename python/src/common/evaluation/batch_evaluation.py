...
class SubprocessRunner:
    def __init__(self, command):
        ...
        import shlex
        self.cmd_args = shlex.split(command)

    def invoke(self, state):
        import subprocess
        # Convert state messages to dicts if they are objects
        # input state has "messages" which are already objects in evaluate_single_instance
        # We need to serialize them to JSON format expected by Go agent
        # Go agent expects "messages": [{"role":..., "content":...}]
        
        json_state = {
            "order": state.get("order"), # Pass order if present
            "messages": []
        }
        ...
        for m in state["messages"]:
            ...
            # Handle tool calls if any (mostly for assistant, but typically input messages are user/system in simple eval)
            json_state["messages"].append(msg_dict)

        input_str = json.dumps(json_state)
        
        try:
            # Run the command with pipes
            # Command should be e.g. "go run cmd/..." or "python -m ..."
            # We append --eval flag if not present? No, assume user provides full command + --eval
            # Actually, user might just provide "go run .../main.go", we might need to assume it accepts stdin.
            # My Go implementation requires --eval.
            ...
            # Convert output messages back to LC objects
            final_messages = []
            for m_dict in output_json.get("messages", []):
                role = m_dict.get("role")
                content = m_dict.get("content", "")
                
                if role == "assistant":
                    # Check for tool calls
                    # Eino tool calls -> [{"function": {"name":..., "arguments":...}, "id":...}]
                    # LangChain AIMessage expects tool_calls=[{'name':..., 'args':..., 'id':...}]
                    tool_calls = []
                    ...
                    # Also populate additional_kwargs for backward compat if needed by evaluation logic
                    msg.additional_kwargs["tool_calls"] = m_dict.get("tool_calls", [])
                    final_messages.append(msg)
                elif role == "tool":
                    final_messages.append(ToolMessage(content=content, tool_call_id=m_dict.get("tool_call_id", "")))
                elif role == "user":
                    final_messages.append(HumanMessage(content=content))
                elif role == "system":
                    final_messages.append(SystemMessage(content=content))
                else:
                    final_messages.append(HumanMessage(content=content))
            ...
            raise e
             ...
             print(f"[ERROR] Failed to decode JSON from subprocess: {e}")
             raise e

def run_evaluation(
    runner, args, weights: Dict[str, float],
    metrics: Dict[str, List[float]], verbose: bool = False
):
    for raw in pathlib.Path(args.dataset).read_text().splitlines():
        result = evaluate_single_instance(raw, runner)
        if result is None:
            try:
...
def main():
    ...
    ap.add_argument("--graph_py", help="Path to Python graph file")
    ap.add_argument("--graph_cmd", help="Command to run external agent (e.g. 'go run ... --eval')")
    ...
    if args.graph_py:
        runner = PythonGraphRunner(args.graph_py)
    elif args.graph_cmd:
        runner = SubprocessRunner(args.graph_cmd)
    else:
        print("Error: Must provide either --graph_py or --graph_cmd")
        return
    ...
    run_evaluation(runner, args, weights, metrics, verbose=args.verbose)

'''
Example usage
python -m src.common.evaluation.batch_evaluation \
  --dataset ../../scenarios/ecommerce_customer_support_evaluation_set.jsonl \
  --graph_py src/frameworks/langgraph_agents/ecommerce_customer_support/customer_support_agent.py

python -m src.common.evaluation.batch_evaluation \
  --dataset ../../scenarios/ecommerce_customer_support_evaluation_set.jsonl \
  --graph_cmd "go run ../go/cmd/ecommerce_customer_support/main.go --eval"
'''
